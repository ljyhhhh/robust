...
def pgd_delta_a(
    ...
    #return delta_a by minimizing L_cond
    ...
    return perturbed_images-original_images #返回delta_a
...
def pgd_attack(
    ...
    """Return new perturbed data with delta_u added"""
    ...
    t=transforms.RandomRotation(22.5) # transformation T
    delta_u=torch.zeros_like(perturbed_images) # initialize delta_u
    for step in range(num_steps):
        perturbed_images.requires_grad = True
        perturbed_images_trans=t(perturbed_images) # transform perturbed images 
        ...
        unet.zero_grad()
        text_encoder.zero_grad()
        loss = loss*0.6 + 0.4*F.mse_loss(model_pred.float(), target.float(), reduction="mean")
        ...
        delta_u=delta_u+alpha * perturbed_images.grad.sign()
        if(step+1)%6==0:
            adv_images = perturbed_images + delta_u/10
            eta = torch.clamp(adv_images - original_images, min=-eps, max=+eps)
            perturbed_images = torch.clamp(original_images + eta, min=-1, max=+1).detach_()
