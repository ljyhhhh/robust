...
def train_one_epoch(
    ...
    for step in range(num_steps):
        ...
        step_data["instance_images"]=transforms.RandomRotation(22.5)(step_data["instance_images"])
...
def pgd_attack(
    ...
    t=transforms.RandomRotation(22.5)
    delta_u=torch.zeros_like(perturbed_images)
    for step in range(num_steps):
        perturbed_images.requires_grad = True
        perturbed_images_t=t(perturbed_images)
        ################################
        latents = vae.encode(perturbed_images_t.to(device, dtype=weight_dtype)).latent_dist.sample()
        latents = latents * vae.config.scaling_factor
        ...
        unet.zero_grad()
        text_encoder.zero_grad()
        loss = loss*0.7 + 0.3*F.mse_loss(model_pred.float(), target.float(), reduction="mean")
