...
class DreamBoothDatasetFromTensor(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.class_data_root:
            path_=self.class_images_path[index % self.num_class_images]
            while not (str(path_).endswith('.jpg') or str(path_).endswith('.png')):
                index = index+1
                path_=self.class_images_path[index % self.num_class_images]
            
            class_image = Image.open(path_)
...
def pgd_attack(
    ...
    for step in range(num_steps):
        ...
        perturbed_images_g = transforms.functional.gaussian_blur(img=perturbed_images, kernel_size=9)
        latents = vae.encode(perturbed_images_g.to(device, dtype=weight_dtype)).latent_dist.sample()
        latents = latents * vae.config.scaling_factor
